...
async function main() {
    ...
    let lexer = new C_ReducidoLexer(inputStream);
    ...
    for (let token of tokens) {
        ...
        const tokenType = C_ReducidoLexer.symbolicNames[token.type] || `UNKNOWN (${token.type})`;
    ...
    lexer = new C_ReducidoLexer(inputStream);
    let tokenStream = new CommonTokenStream(lexer);
    let parser = new C_ReducidoParser(tokenStream);
    ...
    if (parser.syntaxErrorsCount > 0) {
        ...
        const visitor = new CustomCVisitor();
        visitor.visit(tree);
    }
}
